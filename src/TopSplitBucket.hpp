...
#ifndef __TOP_BUCKET_HPP__
#define __TOP_BUCKET_HPP__

#include <stdlib.h>
#include <tuple>

template <typename data_T, typename attribute_index_T> class TopSplitBucket {
  public:
    ...
    bool add(attribute_index_t attributeIndex, data_t splitValue, data_t G) {
        if (G > this->G[0]) {
            ...
            _updateCandidate(0, this->attributeIndex[1], this->splitValue[1],
                             this->G[1]);
  ...
  private:
    attribute_index_t attributeIndex[2] = {0};
